...
Тарифы Мист:- 15 % от фактической стоимости посылки- 14 % - если объем ваших посылок >= 50к $- 20 % - застрявшие посылки на таможне , по ним гарантии нет, так как есть нюансы , каждая такая посылка рассматривается индивидуально, если не получается, то делаем возврат.
...
📩 По любим вопросам пишите @staffship`,
      ...
      if (msg.text === '📩 Написать нам') {
        await bot.sendMessage(chat.id, `@staffship`, {
          parse_mode: 'HTML',
        });
      }
      if (msg.text === '📰 Канал') {
        await bot.sendMessage(chat.id, `@staffshipcc`, {
          parse_mode: 'HTML',
        });
      }
